...
export default function YtDetailModal({ channel: ch, onClose }) {
  if (!ch) return null;
  ...
  const data = [
    ...
    { label: 'View/Sub Ratio', value: ch.viewSubRatio.toFixed(2) },
  ];
          ...
          <div className="detail-sub">
            ...
            <div style={{ background: 'var(--glass)', padding: 12, borderRadius: 'var(--radius-sm)', marginTop: 8 }}>
              ...
              <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                Views: <strong style={{ color: 'var(--accent)' }}>{formatNum(ch.topVideoViews)}</strong>
                {' '}| Đăng: {(ch.publishedAt || '').split('T')[0]}
              </div>
              {ch.topVideoUrl && (
                <a
                  href={ch.topVideoUrl}
