...
export function exportKeywordsCSV(keywords: Keyword[]): string {
  const BOM = '\uFEFF';
  const rows: string[] = [KW_HEADERS.join(',')];
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    results.push({
      ...
      metadata: {
        ...
        collectedAt: c[23] || new Date().toISOString(),
        timeWindowDays: +c[24] || 180,
        regionCode: c[25] || 'JP',
        languageCode: c[26] || 'ja',
        confidenceLevel: (c[27] as 'High' | 'Medium' | 'Low') || 'Low',
        freshnessStatus: (c[28] as 'Fresh' | 'Stale' | 'Old') || 'Old',
      },
    });
  }
  return results;
